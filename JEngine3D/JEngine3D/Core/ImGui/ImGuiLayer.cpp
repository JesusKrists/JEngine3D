...
namespace JE {
    ...
    static void JEngine3DImGuiCreateWindow(ImGuiViewport* viewport)
    {
        ...
        auto& window =
        WindowController::Get().CreateWindow("Temporary ImGui Window Title",
    ...
    static void InitializeImGuiForJEngine3D()
    {
        ...
        imguiIO.SetClipboardTextFn = [](void*, const char* text) { IPlatform::Get().SetClipboardText(text); };
        imguiIO.GetClipboardTextFn = [](void*) { return IPlatform::Get().ClipboardText(); };
        ...
        int monitorCount = IPlatform::Get().MonitorCount();
        for (int i = 0; i < monitorCount; i++) {
            auto bounds       = IPlatform::Get().DisplayBounds(i);
            auto usableBounds = IPlatform::Get().DisplayUsableBounds(i);
            ...
            monitor.DpiScale = IPlatform::Get().DisplayDPI(i) / 96.0F;// NOLINT
    ...
    void ImGuiLayer::OnUpdate()
    {
        ...
        if (MouseButtonsPressed()) {
            IPlatform::Get().CaptureMouse();
        } else {
            IPlatform::Get().ReleaseMouse();
        }
    }
    ...
    void ImGuiLayer::OnEvent(IEvent& event)
    {
        ...
        dispatcher.Dispatch<EventType::TextInput>([&](const IEvent& evnt) {
            const auto& textInputEvent = static_cast<const TextInputEvent&>(evnt);// NOLINT(cppcoreguidelines-pro-type-static-cast-downcast)
        ...
        dispatcher.Dispatch<EventType::MousePress>([&](const IEvent& evnt) {
            const auto& pressEvent = static_cast<const MousePressEvent&>(evnt);// NOLINT(cppcoreguidelines-pro-type-static-cast-downcast)
        ...
        dispatcher.Dispatch<EventType::MouseMove>([&](const IEvent& evnt) {
            const auto& moveEvent = static_cast<const MouseMoveEvent&>(evnt);// NOLINT(cppcoreguidelines-pro-type-static-cast-downcast)
        ...
        dispatcher.Dispatch<EventType::MouseWheel>([&](const IEvent& evnt) {
            const auto& wheelEvent = static_cast<const MouseWheelEvent&>(evnt);// NOLINT(cppcoreguidelines-pro-type-static-cast-downcast)
    ...
    auto
    ImGuiLayer::AddFont(const std::filesystem::path& fontFilePath, float pixelSize, const ImFontConfig* fontCfg, const ImWchar* glyphRanges)
    -> ImFont*
    {
        auto fontIt = FindIf(m_FontCache, [&](const Scope<FontEntry, MemoryTag::App>& entry) {
            return entry->FilePath == fontFilePath.generic_string() && entry->Font->FontSize == pixelSize;
        });
        if (fontIt == std::end(m_FontCache)) {
            m_FontCache.push_back(CreateScope<FontEntry, MemoryTag::App>(fontFilePath.generic_string()));
    ...
    auto ImGuiLayer::AddImGuiInternalFont(const ImFontConfig* fontCfg) -> ImFont*
    {
        auto fontIt = FindIf(m_FontCache, [&](const Scope<FontEntry, MemoryTag::App>& entry) {
            if (fontCfg != nullptr) {
        ...
        if (fontIt == std::end(m_FontCache)) {
            ...
            CreateScope<FontEntry, MemoryTag::App>(fontCfg == nullptr ? IMGUI_INTERNAL_FONT_STRING : IMGUI_INTERNAL_FONT_STRING_CUSTOM));
            auto& entry = m_FontCache.back();
    ...
    void ImGuiLayer::PushMergeFont(const std::filesystem::path&    fontFilePath,
                                   float                           pixelSize,
                                   float                           xAdvance,
    ...
    void ImGuiLayer::PopMergeFont()
    {
        if (!m_MergeFontStack.empty()) { m_MergeFontStack.erase(--std::end(m_MergeFontStack)); }
    }

    void ImGuiLayer::AddMergeFont(ImFontAtlas& fontAtlas)
    {
