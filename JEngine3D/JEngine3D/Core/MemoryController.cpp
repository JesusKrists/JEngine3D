...
namespace JE {
    ...
    void MemoryController::Deallocate(void* memory, MemoryTag tag)
    {
        ...
        auto memoryEntry = FindIf(memoryEntryContainer, [&memory](const MemoryEntry& entry) { return entry.Memory == memory; });
        ...
        if (memoryEntry->Tag != tag) { spdlog::critical("Deallocate: MemoryTag mismatch for memory 0x{}", memoryEntry->Memory); }
