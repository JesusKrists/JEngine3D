...
namespace JE {
    ...
    static constexpr auto SDLKeyCodeToJEngine3DKeyCode(SDL_Keycode keyCode) -> KeyCode { return static_cast<KeyCode>(keyCode); }
    ...
    auto SDLPlatform::Initialized() -> bool { return s_Initialized; }

    auto SDLPlatform::CreateWindow(const std::string_view&    title,
                                   const Size2DI&             size,
                                   ...
                                   const WindowConfiguration& config) -> NativeWindowHandle
    {
        ...
        position != WINDOW_CENTER_POSITION ? position.X : static_cast<int32_t>(SDL_WINDOWPOS_CENTERED),// NOLINT(hicpp-signed-bitwise)
        position != WINDOW_CENTER_POSITION ? position.Y : static_cast<int32_t>(SDL_WINDOWPOS_CENTERED),// NOLINT(hicpp-signed-bitwise)
        size.Width,
        size.Height,
        sdlFlags);
    }

    void SDLPlatform::DestroyWindow(NativeWindowHandle handle)
    {
        ASSERT(handle, "Window handle is null");
        SDL_DestroyWindow(static_cast<SDL_Window*>(handle));
    }

    auto SDLPlatform::ValidWindowHandle(NativeWindowHandle handle) -> bool
    {
        ASSERT(handle, "Window handle is null");
        return SDL_GetWindowID(static_cast<SDL_Window*>(handle)) != 0;
    }

    auto SDLPlatform::WindowSize(NativeWindowHandle handle) -> Size2DI
    {
        ASSERT(handle, "Window handle is null");
        Size2DI size{};
    ...
    void SDLPlatform::SetWindowSize(NativeWindowHandle handle, const Size2DI& size)
    {
        ASSERT(handle, "Window handle is null");
        ASSERT(size.Width > 0 && size.Height > 0, "Window size cannot be zero or negative");
    ...
    auto SDLPlatform::WindowTitle(NativeWindowHandle handle) -> std::string_view
    {
        ASSERT(handle, "Window handle is null");
        return SDL_GetWindowTitle(static_cast<SDL_Window*>(handle));
    }

    void SDLPlatform::SetWindowTitle(NativeWindowHandle handle, const std::string_view& title)
    {
        ASSERT(handle, "Window handle is null");
        SDL_SetWindowTitle(static_cast<SDL_Window*>(handle), std::string(title).c_str());
    }

    auto SDLPlatform::WindowPosition(NativeWindowHandle handle) -> Position2DI
    {
        ASSERT(handle, "Window handle is null");
    ...
    void SDLPlatform::SetWindowPosition(NativeWindowHandle handle, const Position2DI& position)
    {
        ASSERT(handle, "Window handle is null");
    ...
    auto SDLPlatform::WindowHidden(NativeWindowHandle handle) -> bool
    {
        ASSERT(handle, "Window handle is null");
        return (SDL_GetWindowFlags(static_cast<SDL_Window*>(handle)) & SDL_WINDOW_HIDDEN) == SDL_WINDOW_HIDDEN;
    }

    void SDLPlatform::ShowWindow(NativeWindowHandle handle)
    {
        ASSERT(handle, "Window handle is null");
        SDL_ShowWindow(static_cast<SDL_Window*>(handle));
    }

    void SDLPlatform::HideWindow(NativeWindowHandle handle)
    {
        ASSERT(handle, "Window handle is null");
        SDL_HideWindow(static_cast<SDL_Window*>(handle));
    }

    auto SDLPlatform::WindowFocused(NativeWindowHandle handle) -> bool
    {
        ASSERT(handle, "Window handle is null");
        return (SDL_GetWindowFlags(static_cast<SDL_Window*>(handle)) & SDL_WINDOW_INPUT_FOCUS) == SDL_WINDOW_INPUT_FOCUS;
    }

    void SDLPlatform::FocusWindow(NativeWindowHandle handle)
    {
        ASSERT(handle, "Window handle is null");
    ...
    auto SDLPlatform::FocusedWindow() -> NativeWindowHandle { return SDL_GetKeyboardFocus(); }

    auto SDLPlatform::WindowMinimized(NativeWindowHandle handle) -> bool
    {
        ASSERT(handle, "Window handle is null");
        return (SDL_GetWindowFlags(static_cast<SDL_Window*>(handle)) & SDL_WINDOW_MINIMIZED) == SDL_WINDOW_MINIMIZED;
    }

    void SDLPlatform::MinimizeWindow(NativeWindowHandle handle) { SDL_MinimizeWindow(static_cast<SDL_Window*>(handle)); }

    void SDLPlatform::MaximizeWindow(NativeWindowHandle handle) { SDL_RestoreWindow(static_cast<SDL_Window*>(handle)); }

    auto SDLPlatform::MonitorCount() -> int32_t { return SDL_GetNumVideoDisplays(); }

    auto SDLPlatform::DisplayBounds(int32_t displayIndex) -> RectangleI
    {
        SDL_Rect   sdlRect;
        RectangleI rect{};
    ...
    auto SDLPlatform::DisplayUsableBounds(int32_t displayIndex) -> RectangleI
    {
        SDL_Rect   sdlRect;
        RectangleI rect{};
    ...
    auto SDLPlatform::DisplayDPI(int32_t displayIndex) -> float
    {
        float dpi = 0;
    ...
    void SDLPlatform::CaptureMouse() { SDL_CaptureMouse(SDL_TRUE); }

    void SDLPlatform::ReleaseMouse() { SDL_CaptureMouse(SDL_FALSE); }

    void SDLPlatform::PollEvents()// NOLINT(readability-function-cognitive-complexity)
    {
        ...
        while (SDL_PollEvent(&nativeEvent) != 0) {
            ...
            if (nativeEvent.type == SDL_EventType::SDL_WINDOWEVENT) {
                ...
                if (window != nullptr) {
                    switch (nativeEvent.window.event) {
                        case SDL_WindowEventID::SDL_WINDOWEVENT_SIZE_CHANGED: ProcessWindowResizeEvent(nativeEvent); break;

                        case SDL_WindowEventID::SDL_WINDOWEVENT_CLOSE: ProcessWindowCloseEvent(nativeEvent); break;

                        case SDL_WindowEventID::SDL_WINDOWEVENT_MOVED: ProcessWindowMoveEvent(nativeEvent); break;

                        case SDL_WindowEventID::SDL_WINDOWEVENT_HIDDEN: ProcessWindowHideEvent(nativeEvent); break;

                        case SDL_WindowEventID::SDL_WINDOWEVENT_SHOWN: ProcessWindowShowEvent(nativeEvent); break;

                        case SDL_WindowEventID::SDL_WINDOWEVENT_FOCUS_GAINED: ProcessWindowFocusGainedEvent(nativeEvent); break;

                        case SDL_WindowEventID::SDL_WINDOWEVENT_FOCUS_LOST: ProcessWindowFocusLostEvent(nativeEvent); break;

                        case SDL_WindowEventID::SDL_WINDOWEVENT_MINIMIZED: ProcessWindowMinimizedEvent(nativeEvent); break;

                        case SDL_WindowEventID::SDL_WINDOWEVENT_MAXIMIZED: ProcessWindowMaximizedEvent(nativeEvent); break;

                        case SDL_WindowEventID::SDL_WINDOWEVENT_RESTORED: ProcessWindowRestoredEvent(nativeEvent); break;
            ...
            if (nativeEvent.type == SDL_EventType::SDL_TEXTINPUT) {
                TextInputEvent event{ SDL_GetWindowFromID(nativeEvent.text.windowID), static_cast<char*>(nativeEvent.text.text) };
                EventProcessor().OnEvent(event);
            }
            ...
            if (nativeEvent.type == SDL_EventType::SDL_MOUSEWHEEL) {
                MouseWheelEvent event{ SDL_GetWindowFromID(nativeEvent.wheel.windowID),
                                       nativeEvent.wheel.direction == SDL_MOUSEWHEEL_NORMAL ? nativeEvent.wheel.y
                                                                                            : nativeEvent.wheel.y * -1 };
                EventProcessor().OnEvent(event);
            }
        }
    }
    ...
    void SDLPlatform::PushEvent(IEvent& event)
    {
        ...
        if (event.Type() == EventType::WindowClose) {
            const auto& closeEvent = static_cast<const WindowCloseEvent&>(event);// NOLINT(cppcoreguidelines-pro-type-static-cast-downcast)
        ...
        if (event.Type() == EventType::WindowMove) {
            const auto& moveEvent = static_cast<const WindowMoveEvent&>(event);// NOLINT(cppcoreguidelines-pro-type-static-cast-downcast)
        ...
        if (event.Type() == EventType::WindowHide) {
            const auto& hideEvent = static_cast<const WindowHideEvent&>(event);// NOLINT(cppcoreguidelines-pro-type-static-cast-downcast)
        ...
        if (event.Type() == EventType::WindowShow) {
            const auto& showEvent = static_cast<const WindowShowEvent&>(event);// NOLINT(cppcoreguidelines-pro-type-static-cast-downcast)
        ...
        if (event.Type() == EventType::KeyPress) {
            const auto& keyPressEvent = static_cast<const KeyPressEvent&>(event);// NOLINT(cppcoreguidelines-pro-type-static-cast-downcast)
        ...
        if (event.Type() == EventType::TextInput) {
            ...
            textInputEvent.Text().copy(static_cast<char*>(nativeTextInputEvent.text.text), sizeof(nativeTextInputEvent.text.text));
        ...
        if (event.Type() == EventType::MouseRelease) {
            ...
            nativeMouseReleaseEvent.button.windowID = SDL_GetWindowID(static_cast<SDL_Window*>(mouseReleaseEvent.WindowHandle()));
    ...
    auto SDLPlatform::CurrentTicks() -> uint64_t { return SDL_GetPerformanceCounter(); }

    auto SDLPlatform::TickFrequency() -> uint64_t { return SDL_GetPerformanceFrequency(); }

    void SDLPlatform::SetClipboardText(const std::string_view& text) { SDL_SetClipboardText(std::string(text).c_str()); }

    auto SDLPlatform::ClipboardText() -> const char*
    {
        if (m_ClipboardText != nullptr) {
            SDL_free(m_ClipboardText);// NOLINT(cppcoreguidelines-no-malloc,hicpp-no-malloc,cppcoreguidelines-owning-memory)
        }
    ...
    void SDLPlatform::Delay(uint32_t milliseconds) { SDL_Delay(milliseconds); }
