...
namespace JE {
    ...
    {
        switch (format) {
            case FramebufferAttachmentFormat::RGBA8: return GL_RGBA;
            case FramebufferAttachmentFormat::RED_INTEGER: return GL_RED_INTEGER;
            case FramebufferAttachmentFormat::DEPTH24STENCIL8: return GL_DEPTH_STENCIL;
    ...
    auto OpenGLFramebuffer::ReadPixel(uint32_t attachmentIndex, const Position2DI& pixel) -> int
    {
        ...
        int pixelData = 0;
        glReadPixels(pixel.X, pixel.Y, 1, 1, GL_RED_INTEGER, GL_INT, &pixelData);// This probably only works for GL_RED_INTEGER buffers
        return pixelData;
    }
    ...
    void
    OpenGLFramebuffer::AttachTexture(uint32_t attachmentSlot, FramebufferAttachmentFormat format, const Size2DI& size, uint32_t samples)
    {
