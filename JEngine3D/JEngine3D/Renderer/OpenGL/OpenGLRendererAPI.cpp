...
namespace JE {
                                                       ...
                                                       const void* userParam)// NOLINT
    {
        ...
        switch (severity) {// NOLINT
            case GL_DEBUG_SEVERITY_HIGH: JE::Logger::CoreLogger().critical(message); return;
            case GL_DEBUG_SEVERITY_MEDIUM: JE::Logger::CoreLogger().error(message); return;
            case GL_DEBUG_SEVERITY_LOW: JE::Logger::CoreLogger().warn(message); return;
            case GL_DEBUG_SEVERITY_NOTIFICATION: JE::Logger::CoreLogger().trace(message); return;
        }
    ...
    void OpenGLRendererAPI::SetViewport(const RectangleI& viewport)
    {
        ...
        if (RendererState().ClipTest) { glScissor(viewport.Position.X, viewport.Position.Y, viewport.Size.Width, viewport.Size.Height); }
    }
