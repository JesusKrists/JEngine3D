...
namespace JE {
    ...
    static constexpr std::array<glm::vec2, 4> QUAD_UV_COORDINATES = {
    ...
    Renderer2D::Renderer2D()
    {
        ...
        Data.Shader = IRendererObjectCreator::Get().CreateShader("Renderer2D Shader", VERTEX_SHADER_SOURCE, FRAGMENT_SHADER_SOURCE);
    }
    ...
    void Renderer2D::DrawQuad(const glm::vec3& position, const glm::vec2& size, const Color& color)
    {
        ...
        glm::mat4 transform = glm::translate(glm::mat4(1.0F), position) * glm::scale(glm::mat4(1.0F), { size.x, size.y, 1.0F });// NOLINT
        DrawQuad(transform, color);
    }

    void Renderer2D::DrawQuad(const glm::vec3& position, const glm::vec2& size, const ITexture2D& texture, const Color& tintColor)
    {
    ...
    void Renderer2D::DrawQuad(const glm::mat4& transform, const ITexture2D& texture, const Color& tintColor)
    {
        ...
        Data.TriangleVertices.emplace_back(transform * QUAD_VERTEX_POSITION[0], tintColor, QUAD_UV_COORDINATES[0], textureIndex);
        Data.TriangleVertices.emplace_back(transform * QUAD_VERTEX_POSITION[1], tintColor, QUAD_UV_COORDINATES[1], textureIndex);
        Data.TriangleVertices.emplace_back(transform * QUAD_VERTEX_POSITION[2], tintColor, QUAD_UV_COORDINATES[2], textureIndex);
        Data.TriangleVertices.emplace_back(transform * QUAD_VERTEX_POSITION[3], tintColor, QUAD_UV_COORDINATES[3], textureIndex);
